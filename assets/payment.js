(function () {
  ...
  async function fetchOrder(orderId, refresh = false) {
    ...
    if (api.configured) {
      try {
        ...
        const res = await fetch(api.url(path), { cache: 'no-store' });

        if (res.ok) {
          const remoteOrder = await res.json();
          persistLocalOrder(remoteOrder);
          return remoteOrder;
        }

        if (res.status === 404 && local) {
          console.warn('Pedido não encontrado no backend. Usando cópia local.', orderId);
          return local;
        }

        const data = await res.json().catch(() => ({}));
        if (local) {
          console.warn('Falha ao consultar backend. Usando cópia local.', data);
          return local;
        }

        throw new Error(data.error || 'Não foi possível consultar o pedido no backend.');
      } catch (err) {
        if (local) {
          console.warn('Erro ao consultar backend. Usando cópia local.', err);
          return local;
        }
        throw err;
      }
  ...
  async function refreshPayment(orderId) {
    ...
    if (api.configured) {
      try {
        const res = await fetch(api.url(`/api/orders/${encodeURIComponent(orderId)}/refresh-payment`), {
          method: 'POST'
        });

        if (res.ok) {
          const updated = await res.json();
          persistLocalOrder(updated);
          return updated;
        }
  ...
  async function simulateApproval(orderId) {
    ...
    if (api.configured) {
      const res = await fetch(api.url(`/api/orders/${encodeURIComponent(orderId)}/simulate-approve`), {
        method: 'POST'
      });
  ...
  function renderOrder(order) {
    ...
    if ($('#paymentStatusBadge')) $('#paymentStatusBadge').textContent = statusBadgeText(order.status);
    if ($('#paymentStatusTitle')) $('#paymentStatusTitle').textContent = statusTitle(order.status);
    ...
    if ($('#paymentOrderId')) $('#paymentOrderId').textContent = order.orderId || '-';
    if ($('#paymentMethod')) $('#paymentMethod').textContent = order.paymentMethod || '-';
    if ($('#paymentCustomer')) $('#paymentCustomer').textContent = order.customer?.playerNick || '-';
    if ($('#paymentEmail')) $('#paymentEmail').textContent = order.customer?.email || '-';
    if ($('#paymentSubtotal')) $('#paymentSubtotal').textContent = brl(order.totals?.subtotal || 0);
    if ($('#paymentDiscount')) $('#paymentDiscount').textContent = `- ${brl(order.totals?.discount || 0)}`;
    if ($('#paymentTotal')) $('#paymentTotal').textContent = brl(order.totals?.total || 0);
    ...
    const delText = (v) => ({
      ...
      queued: "Na fila de entrega",
      failed: "Falha na entrega",
      needs_config: "Precisa configurar catálogo",
      not_generated: "Aguardando aprovação"
    }[String(v || "").toLowerCase()] || "Aguardando");
    ...
    if ($('#pixCode')) $('#pixCode').value = order.payment?.qrCodeText || '';
    renderQr(order);
    ...
    if (refreshBtn) refreshBtn.classList.toggle('hidden', paid);

    const simulateBtn = $('#simulateApprovalBtn');
    const localMode = !((window.RedeCatsAPI || {}).configured);
    if (simulateBtn) simulateBtn.classList.toggle('hidden', paid || !localMode);
    ...
    if (trust) {
      ...
      if (order.payment?.dateOfExpiration) {
        details.push(`⏳ expira em ${new Date(order.payment.dateOfExpiration).toLocaleString('pt-BR')}`);
      }
